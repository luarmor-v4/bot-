...
const config = {
    ...
    workink: {
        ...
        validationUrl: process.env.WORKINK_VALIDATION_URL || 'https://work.ink/29pu/validation',
        keySystemUrl: process.env.WORKINK_KEY_SYSTEM_URL || 'https://work.ink/29pu/key-sistem-3'
    }
};
...
class WorkInkAPI {
    ...
    // Generate admin key dari Work Ink API (direct, tanpa checkpoint)
    async generateAdminKey(hwid) {
        try {
            // Coba GET request ke API dengan admin mode
            const response = await axios.get(this.keySystemUrl, {
                params: {
                    hwid: hwid,
                    zone: 'Asia/Jakarta',
                    ts: Date.now(),
            ...
            // Fallback: Coba POST request
            const postResponse = await axios.post(this.keySystemUrl, {
                hwid: hwid,
                zone: 'Asia/Jakarta',
                apiKey: this.apiKey,
                admin: true,
            ...
            return {
                success: false,
                error: 'Failed to generate key from API'
            };
        } catch (error) {
            return {
                success: false,
...
const commands = [
    ...
    new SlashCommandBuilder()
        .setName('stats')
        .setDescription('Statistik penggunaan bot (Admin)')
        .setDefaultMemberPermissions(PermissionFlagsBits.Administrator),

    new SlashCommandBuilder()
        .setName('resetuser')
        .setDescription('Reset data user (Admin)')
        .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    ...
    new SlashCommandBuilder()
        .setName('broadcast')
        .setDescription('Kirim pesan ke semua user (Admin)')
        .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
        .addStringOption(option =>
...
async function handleSlashCommand(interaction) {
    ...
    if (commandName === 'ping') {
        ...
        await interaction.reply({
            content: `Pong! Bot online dan responsif. Latency: ${ping}ms`,
            ephemeral: true
        });
        console.log(`[PING] ${interaction.user.username} pinged bot. Latency: ${ping}ms`);
    }
    ...
    // ─────────────────────────────────────────
        ...
        if (isAdmin) {
            // Admin bypass - dapatkan key dari Work Ink API
            const adminKeyResult = await workInk.generateAdminKey(hwid);
            
            if (adminKeyResult.success && adminKeyResult.key) {
                const directKey = adminKeyResult.key;
                ...
                const embed = new EmbedBuilder()
                    .setColor(0xFFD700)
                    .setTitle('🔐 Admin Key Direct')
                    .setDescription('Key dari Work Ink API - tanpa checkpoint')
                    .addFields(
                        { 
                            name: '🔑 Your Key', 
                            value: `\`\`\`${directKey}\`\`\``, 
                        ...
                        {
                            name: '⭐ Status',
                            value: 'Admin Bypass Active',
                            inline: true
                        }
                    )
                    .setFooter({ text: 'Admin Key - No checkpoint required' })
                    .setTimestamp();
                ...
                console.log(`[GETKEY-ADMIN] ${interaction.user.username} generated admin key for HWID: ${hwid.substring(0, 10)}...`);
            } else {
                // Jika API gagal, tampilkan error
                const errorEmbed = new EmbedBuilder()
                    .setColor(0xFF0000)
                    .setTitle('❌ Gagal Generate Key')
                    .setDescription('Tidak bisa generate key dari Work Ink API')
                    .addFields({
                        name: 'Error',
                        value: adminKeyResult.error || 'Unknown error'
                    })
                    .setFooter({ text: 'Admin Key Generation Failed' })
                    .setTimestamp();
                ...
                console.log(`[GETKEY-ADMIN-ERROR] ${interaction.user.username} - API Error: ${adminKeyResult.error}`);
            }
        } else {
            // User biasa - tampilkan link dengan button
    ...
    // ─────────────────────────────────────────
    // /resetuser - Reset user data (Admin Only)
    // ─────────────────────────────────────────
        ...
        if (db.keys[targetUser.id]) {
            delete db.keys[targetUser.id];
        }
        saveDatabase(db);

        await interaction.reply({
            content: `✅ Data user ${targetUser.username} berhasil direset!`,
            ephemeral: true
        });
    ...
    // ─────────────────────────────────────────
        ...
        await interaction.reply({
            content: '✅ Broadcast berhasil dikirim!',
            ephemeral: true
        });
        ...
        console.log(`[BROADCAST] ${interaction.user.username}: ${message}`);
    }
}
...
async function handleButton(interaction) {
    if (interaction.customId === 'howto_key') {
        const embed = new EmbedBuilder()
            .setColor(0x5865F2)
            .setTitle('❓ Cara Mendapatkan Key')
            .setDescription('Panduan lengkap untuk mendapatkan key.')
            .addFields({
                name: '📝 Langkah-langkah',
                value:
                    '```\n' +
                    '1. Klik tombol "Ambil Key"\n' +
                    '2. Akan terbuka website Work Ink\n' +
                    '3. Selesaikan 2-3 checkpoint\n' +
                    '4. Di akhir, key akan muncul\n' +
                    '5. Copy key tersebut\n' +
                    '6. Paste ke script kamu\n' +
                    '```'
            })
            .addFields({
                name: '⚠️ Tips',
                value:
                    '• Jangan skip checkpoint, ikuti prosesnya\n' +
                    '• Key berlaku 24 jam\n' +
                    '• Jika expired, request key baru dengan /getkey'
            })
            .setFooter({ text: 'Work Ink Key System' })
            .setTimestamp();
